...
import { useDispatch, useSelector } from 'react-redux'
 ...
 const Nav = ({auth}) => {
  ...
  const { error, isError, isSuccess, isLoading, mutate, data} = useMutation(() => {
    return axios.post('http://localhost:8000/api/user/user-logout', {}, {withCredentials:true})
  })
  const logOut = (e) => {
    e.preventDefault()
    mutate({})
    ...
    <nav className="max-w-7xl flex basis-full items-center w-full mx-auto px-4 sm:px-6 lg:px-8" aria-label="Global">
      ...
      <div className="w-full flex items-center justify-end ml-auto sm:justify-between sm:gap-x-3 sm:order-3">
        ...
        <div className="flex flex-row items-center justify-end gap-2">
          ...
          {profile?.auth && <div className="hs-dropdown relative inline-flex" data-hs-dropdown-placement="bottom-right">
            ...
            <div className="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[15rem] z-10 bg-white shadow-md rounded-lg p-2 dark:bg-gray-800 dark:border dark:border-gray-700" aria-labelledby="hs-dropdown-with-header">
              <div className="py-3 px-5 -m-2 bg-gray-100 rounded-t-lg dark:bg-gray-700">
                ...
                <p className="text-sm font-medium text-gray-800 dark:text-gray-300">{profile?.user?.email}</p>
              </div>
              <div className="mt-2 py-2 first:pt-0 last:pb-0">
                ...
                {isLoading ? "loading" : <span onClick={logOut} className="flex capitalize cursor-pointer items-center gap-x-3.5 py-2 px-3 rounded-md text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300" href="#">
          ...
          {!profile?.auth ? <span className='text-base font-medium capitalize text-gray-700 mx-5 cursor-pointer' onClick={() => dispatch(openModel())}>login</span>: ""}
